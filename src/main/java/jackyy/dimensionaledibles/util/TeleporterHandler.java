...
public class TeleporterHandler {
	...
	while (!foundSpawnPos && pos.getY() > 0) {
		...
		if (world.getBlockState(pos.add(0, -1, 0)).isSideSolid(world, pos.add(0, -1, 0), EnumFacing.UP) && world.isAirBlock(pos) && world.isAirBlock(pos.add(0, 1, 0))) {
		    foundSpawnPos = true;
		} else if (world.getBlockState(pos.add(0, -1, 0)).getBlock() instanceof BlockLiquid && world.isAirBlock(pos) && world.isAirBlock(pos.add(0, 1, 0))) {
		    foundSpawnPos = true;
		}
	    }
	    if (!foundSpawnPos) {
